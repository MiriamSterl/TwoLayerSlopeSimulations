...
def compute_EKE(slope,layer,field,run):
    ...
    if layer=='1': # remove particles trapped in eddies (with anomalously high q values)
        ...
        u_new = np.delete(u,anom,axis=0)
        v_new = np.delete(v,anom,axis=0)
    else:
        u_new = u
        v_new = v

    eke_u = np.mean(u_new**2,axis=0)
    eke_v = np.mean(v_new**2,axis=0)
    return eke_u,eke_v
...
def compute_save_EKE(slope,field):
    ...
    df_eke_u_layer1.to_csv("../../Results/Results_EKE/SmallLd/lagrangian/"+fn_base+"_eke_u_layer1_1hr12hr_highqfilter.csv",index=False)
    df_eke_v_layer1.to_csv("../../Results/Results_EKE/SmallLd/lagrangian/"+fn_base+"_eke_v_layer1_1hr12hr_highqfilter.csv",index=False)
    df_eke_u_layer2.to_csv("../../Results/Results_EKE/SmallLd/lagrangian/"+fn_base+"_eke_u_layer2_1hr12hr_highqfilter.csv",index=False)
    df_eke_v_layer2.to_csv("../../Results/Results_EKE/SmallLd/lagrangian/"+fn_base+"_eke_v_layer2_1hr12hr_highqfilter.csv",index=False)
